 ...
 */
public record DeviceResponse(String deviceId, String platform, String deviceName,
                             Instant firstSeenAt, Instant lastSeenAt) {

    static DeviceResponse of(Device device) {
        return new DeviceResponse(device.getDeviceId(), device.getPlatform(), device.getDeviceName(),
